...
class PhiRotaryEmbedding(tf.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, name=None):
        super().__init__(name)
        print(dim, max_position_embeddings, base) # 32 2048 10000.0
        self.dim = dim
...
class PhiAttention(tf.Module):
    ...
    def __call__(
        ...
        attention_mask = None,
        position_ids = None,
        past_key_value_old = None, # modified
        output_attentions: bool = False,
        use_cache: bool = True):
        bsz, q_len, _ = hidden_states.shape
        print("hidden_states.shape: ", hidden_states.shape) # (1, 11, 2560)
        ...
        # past_key_value = tf.identity(past_key_value_old)
        # past_key_value = past_key_value_old[:]
        if past_key_value_old == []:
            past_key_value = past_key_value_old[:]
        else:
            assert(isinstance(past_key_value_old[0], list)) # assert is a list(list(tensor))
            past_key_value = [kv[:] for kv in past_key_value_old] # deepcopy

        # print("past_key_value after tf.identity:", past_key_value)
        ...
        # print("query_rot after truncations", query_rot)
        # print("key_pass after truncations", key_pass)

        # [batch_size, seq_length, num_heads, head_dim // config.partial_rotary_factor]
        query_rot, key_rot = apply_rotary_pos_emb(query_rot, key_rot, cos, sin, position_ids)

        print("key_rot after apply_rotary_pos_emb", key_rot.shape)
        ...
        if past_key_value is not None:
            # Update the cache
            if len(past_key_value) <= self.layer_idx: # first run
        ...
        print("attn_weights.shape right before applying mask:", attn_weights.shape)
        ...
        # return attn_output, attn_weights # remove later
...
class PhiModel(tf.Module):
    ...
    def __call__(
        ...
        if use_cache:
            if not isinstance(past_key_values, list): # first run, not the perfect test as will fail if list is empty
                assert(past_key_values != [])
                past_key_values = []
            else:
                past_key_values_length = past_key_values[0][0].shape[-2] # seq len
        ...
        next_decoder_cache = past_key_values # modified
        ...
        for decoder_layer in self.layers:
            ...
            layer_outputs = decoder_layer(
                ...
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_value=next_decoder_cache, # modified
                output_attentions=output_attentions,
                use_cache=use_cache,
            )
            ...
            # print("next decoder cache in loop iteration:", next_decoder_cache)
            if output_attentions:
...
class PhiForCausalLM(tf.Module):
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values is not None:
            ...
            print("cache length:", cache_length) # this is wierd
        ...
        if attention_mask is not None and position_ids is None:
            ...
            position_ids = masking_utils_gm.mask_fill(position_ids, tf.cast(attention_mask == 0, tf.int32), 1)
