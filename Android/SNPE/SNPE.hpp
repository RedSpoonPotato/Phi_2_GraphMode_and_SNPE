...
public:
  ...
  bool registerIonBuffers(const DlSystem::UserMemoryMap& ionBufferMap) noexcept{
    return SNPE_SUCCESS == Snpe_SNPE_RegisterIonBuffers(handle(), getHandle(ionBufferMap));
  }

  bool deregisterIonBuffers(const DlSystem::StringList& ionBufferNames) noexcept{
    return SNPE_SUCCESS == Snpe_SNPE_DeregisterIonBuffers(handle(), getHandle(ionBufferNames));
  }
