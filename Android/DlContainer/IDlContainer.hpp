...
public:
  ...
  void getRecord(const std::string& name, DlcRecord& record) const{
    auto h = Snpe_DlContainer_GetRecord(handle(), name.c_str());
    if(!h) throw std::runtime_error("invalid DlcRecord name");
  ...
  void getRecord(const DlSystem::String& name, DlcRecord& record) const{
    getRecord(static_cast<const std::string&>(name), record);
  }
